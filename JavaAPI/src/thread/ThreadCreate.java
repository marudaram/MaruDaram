...
class ThreadEx extends Thread {
	//public void run 이라는 메서드 오버라이딩
	@Override 
	//위의 어노테이션은 상위클래스나 인터페이스에서 제공하는 메서드가 아닌 경우 에러를 발생시켜준다
	public void run() {
		//스레드로 수행할 내용
...
class RunnableImpl implements Runnable {
	...
	public void run() {
		//스레드로 수행할 내용
		//1초마다 Runnable interface 라는 문장을 10번 출력
		for(int i = 0; i < 10; i++) {
			try {
				Thread.sleep(1000);
...
public class ThreadCreate {
	public static void main(String[] args) {
		...
		//Runnable 인터페이스를 Annonymous class를 이용해서 사용
